...
//			set ship:control:pilotmainthrottle to 0.1/maxTWR().
...
declare function phaseshift {
	...
	print "*" at(terminal:width, newphase*2).
	set flightphase to newphase.
}

declare function readout {
//	print "STEERING @ " + round(90-turnangle-pitch,2) at(2,1).
	print "THROTTLE = " + round(thrott,3) at(2,2).
	print "  MAXTWR = " + round(maxTWR(),3) at(2,3).
	print " STAGEDV = " + round(stageDV(),3) at(2,4).
	print " STAGELF = " + stage:liquidfuel at(2,5).
	print " STAGESF = " + stage:solidfuel at(2,6).
}
...
declare function stageMonitor {
	// check stage fuel level for zero
