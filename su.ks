...
// landed altitude
lock surfaceAltitude to ship:geoposition:terrainheight.
lock landedAltitude to surfaceAltitude + 2.
// surface altitude
...
until false
{
	...
	lock gravityAcc to body:mu / (altitude + body:radius)^2.
	...
	until abs(error) < 0.1
	{
		...
		set burnTimeEngine to burnTime-0.05.
	...
	wait until alt:radar < (burnStartAltitude + 50).
}
...
until (alt:radar <= (burnStartAltitude + simulationStep*1.25))
{
...
set altitudeError to alt:radar - burnStartAltitude.
if (altitudeError > 0) {
...
if ((alt:radar - landedAltitude) > 5) {
	...
	wait 0.25.
	lock throttle to ((ship:mass * gravityAcc) / (engine:availablethrust * engineCount))*0.99.
